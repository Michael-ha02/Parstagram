...
//  Parstagram
//
//  Created by Michael Ha on 3/23/22.
//
...
class CameraViewController: UIViewController, UIImagePickerControllerDelegate & UINavigationControllerDelegate {
    ...
    // when you click on the image, the camera gonna show up
    @IBAction func imagePress(_ sender: Any) {
        let picker = UIImagePickerController()
        picker.delegate = self
        picker.allowsEditing = true
        
        // if the camera working, it will default open the camera
    ...
    func imagePickerController(_ picker: UIImagePickerController, didFinishPickingMediaWithInfo info: [UIImagePickerController.InfoKey : Any]) {
        ...
        dismiss(animated: true, completion: nil) // this method will comeback after we done the task
    }
